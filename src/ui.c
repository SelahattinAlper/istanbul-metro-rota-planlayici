...
void DrawStopDropdown(int x, int y, int width, Graph* graph, int selectedLine,
    ...
    // Dropdown açıksa tüm durakları göster
    if (*showDropdown && selectedLine >= 0) {
        ...
        // Tüm duraklar için liste yüksekliği
        int listHeight = stopCount * 30;
        ...
        for (int i = 0; i < stopCount; i++) {
            ...
            DrawText(stopNames[i], x + 10, itemY + 6, 14, WHITE);

            // Tıklama ile seç
            if (isHovered && IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
                *selectedStopId = stopIds[i];
                strncpy(selectedName, stopNames[i], MAX_NAME_LEN - 1);
...
void DrawMainUI(Graph* graph, UIState* state, Node* bst) {
    ...
    // Alt Butonlar - 2x2 ızgara, ortada (özet panelinin üstünde)
    int centerX = MAIN_WINDOW_WIDTH / 2;
    int btnWidth = 180;
    int btnHeight = 45;
    int btnSpacingX = 20;
    int btnSpacingY = 15;
    int gridWidth = 2 * btnWidth + btnSpacingX;
    int btnStartX = centerX - gridWidth / 2;
    int btnY1 = 400;
    int btnY2 = btnY1 + btnHeight + btnSpacingY;

    // Orta Panel - Seçim Özeti (butonların altında)
    int summaryY = btnY2 + btnHeight + 20;

    // Dijkstra Butonu (sol üst)
    Rectangle dijkstraBtn = {btnStartX, btnY1, btnWidth, btnHeight};
    ...
    DrawText("En Kisa Sure", btnStartX + 30, btnY1 + 15, 18, WHITE);
    ...
    // BFS Butonu (sağ üst)
    Rectangle bfsBtn = {btnStartX + btnWidth + btnSpacingX, btnY1, btnWidth, btnHeight};
    ...
    DrawText("En Az Durak", bfsBtn.x + 35, btnY1 + 15, 18, WHITE);

    if (bfsHover && IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
        CalculateRouteForUI(graph, state, false);
    }

    // Temizle Butonu (sol alt)
    Rectangle clearBtn = {btnStartX, btnY2, btnWidth, btnHeight};
    ...
    DrawText("Temizle", clearBtn.x + 55, btnY2 + 15, 18, WHITE);
    ...
    // Çıkış Butonu (sağ alt)
    Rectangle exitBtn = {btnStartX + btnWidth + btnSpacingX, btnY2, btnWidth, btnHeight};
    ...
    DrawText("Cikis", exitBtn.x + 70, btnY2 + 15, 18, WHITE);
    ...
    // --- Yoğunluk Güncelle Butonu (özet panelinin altında) ---
    int refreshBtnY = summaryY + 110;
    Rectangle refreshBtn = {centerX - 90, refreshBtnY, 180, 40};
    bool refreshHover = IsMouseOverRect(refreshBtn.x, refreshBtn.y, refreshBtn.width, refreshBtn.height);
    DrawRectangleRounded(refreshBtn, 0.3f, 8, refreshHover ? (Color){142, 68, 173, 255} : (Color){120, 50, 150, 255});
    DrawText("Yogunluk Guncelle", refreshBtn.x + 15, refreshBtn.y + 12, 14, WHITE);
    
    if (refreshHover && IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
        generate_random_congestion(graph);
    }
